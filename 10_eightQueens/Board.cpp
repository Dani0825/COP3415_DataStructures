...
void Board::place_queens() {
    bool finished = false;
    place_queen_helper(0, 0, 0, finished);
}

void Board::place_queen_helper(int row, int col, int queens_place, bool& finished){
    //Base case
    if (queens_place == 8){
        finished = true;
        return;
        ...
        for (int j = col; j < board_size; j++){
            if (board[i][j] == ' '){
                if (check_place(i, j)){
                    ...
                    if(!finished){
                        print();
                        place_queen_helper(i + 1, 0, queens_place + 1, finished);
                        if (!finished) {
                            board[i][j] = ' ';
                        }
                    }
